...
public record TextureMetaGenerator(List<ResourceLocation> sources, Optional<AnimationData> animation,
                                   Optional<VillagerData> villager, Optional<TextureData> texture,
                                   ResourceLocation outputLocation) implements IResourceGenerator {
        ...
        return () -> {
            ...
            if (sources().isEmpty()) {
                DynamicAssetGenerator.LOGGER.error(
                        "No sources provided for texture metadata at {}:\n",outputLocation());
                return null;
            }
            for (ResourceLocation source : sources()) {
                try (InputStream stream = ClientPrePackRepository.getResource(
                        new ResourceLocation(source.getNamespace(), "assets/dynamic_asset_generator/textures/" +source.getPath()+".png.mcmeta")))
                {
                    JsonObject json = GSON.fromJson(new BufferedReader(
                            new InputStreamReader(stream)), JsonObject.class);
                    MetaStructure structure = MetaStructure.CODEC.parse(JsonOps.INSTANCE, json)
                            .getOrThrow(false, e->{});
                    sourceStructure.put(source,structure);
                } catch (IOException | RuntimeException e) {
                    //Not an error, it might just not have metadata
                    sourceStructure.put(source,new MetaStructure(Optional.empty(),
            ...
            if (!animationMetas.isEmpty()) {
                ...
                List<Integer> frameCount = animationMetas.stream().map(m->m.frames.stream()
                        .max(Integer::compareTo).orElse(0)+1).toList();
                List<Integer> scale = new ArrayList<>(animation().map(AnimationData::scales).map(l->l.orElse(List.of()))
                        .orElse(List.of()));
                List<Integer> relFrameCount = new ArrayList<>();

                Supplier<ResourceLocation> rlFinder = () -> sources().stream()
                        .filter(i->sourceStructure.get(i).animation().map(m->!m.frames.equals(List.of(0)))
                                .orElse(false))
                ...
                if (!sources.contains(patternSourceRl)) {
                    DynamicAssetGenerator.LOGGER.error(
                            "Source specified was not the name of a texture source: {}",patternSourceRl);
                    return null;
                }

                List<Integer> framesSource = sourceStructure.get(patternSourceRl).animation
                        .map(MetaStructure.AnimationMeta::frames).orElse(List.of(0));
                int patternSourceIdx = sourceStructure.get(patternSourceRl).animation.map(animationMetas::indexOf)
                        .orElse(0);
            ...
            JsonElement jsonOut = MetaStructure.CODEC.encodeStart(JsonOps.INSTANCE, out)
                    .getOrThrow(false, e->{});
    ...
    public @NotNull Set<ResourceLocation> getLocations() {
        return Set.of(new ResourceLocation(outputLocation().getNamespace(),
                "assets/dynamic_asset_generator/textures/" +outputLocation().getPath()+".png.mcmeta"));
    }
    ...
    public record AnimationData(Optional<Integer> frametime, Optional<Boolean> interpolate,
                                Optional<ResourceLocation> patternSource, Optional<List<Integer>> scales) {
    ...
    private record MetaStructure(Optional<AnimationMeta> animation, Optional<TextureMeta> texture,
                                 Optional<VillagerMeta> villager) {
        ...
        record AnimationMeta(int frametime, List<Integer> frames, boolean interpolate) {
            public static final Codec<AnimationMeta> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                    ...
                    Codec.BOOL.optionalFieldOf("interpolate",false)
                            .forGetter(AnimationMeta::interpolate)
        ...
        record VillagerMeta(VillagerData.Hat hat) {
            public static final Codec<VillagerMeta> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                    VillagerData.Hat.CODEC.optionalFieldOf("hat", VillagerData.Hat.NONE)
                            .forGetter(VillagerMeta::hat)
