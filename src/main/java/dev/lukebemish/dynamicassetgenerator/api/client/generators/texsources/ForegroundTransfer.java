...
public record ForegroundTransfer(ITexSource background, ITexSource full, ITexSource newBackground,
    ...
    private static final String nonExistentTextureError = "Texture given was nonexistant...\n{}";
    ...
    public Supplier<NativeImage> getSupplier(TexSourceDataHolder data) throws JsonSyntaxException {
        ...
        return () -> {
            try (NativeImage bImg = background.get();
                ...
                if (bImg == null) {
                    data.getLogger().error(ErrorSource.nonExistentErrorF, this.background());
                    return null;
                }
                if (nImg == null) {
                    data.getLogger().error(ErrorSource.nonExistentErrorF, this.newBackground());
                    return null;
                }
                if (fImg == null) {
                    data.getLogger().error(ErrorSource.nonExistentErrorF, this.full());
                    return null;
                }
